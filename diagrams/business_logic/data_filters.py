"""Includes the data filters for pickers and datatables in the diagrams app"""
from datetime import timedelta, datetime
from django.db.models import Q
from diagrams.models import Diagrams

def diagram_picker_filter(value):
    """Given a value, filters diagrams for a select picker"""
    return list(Diagrams.objects.filter(
        Q(name__icontains=value)
    )[:10])

def diagram_listing_filter(filters, count=False):
    ...
    for i in filters:
        ...
        if i['type'] == 'Text':
            if i['value'] != '':
                param = field + ('__icontains' if i['options']['data_type'] == 'text' else '')
